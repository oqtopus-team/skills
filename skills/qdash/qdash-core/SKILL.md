---
name: qdash-core
description: Connect agents to a running QDash instance through qdash-client and saved local profiles. Use when an agent needs QDash profile discovery, safe authentication handling, project context, read-only API helpers, raw read-only endpoint access, or shared QDash access rules before using more specific QDash skills.
---
...
Use this skill to let an agent query QDash through `qdash-client` instead of scraping the UI, calling `curl` directly, or hand-writing auth headers. Prefer saved profiles for credentials and keep tokens, Cloudflare Access secrets, passwords, and raw config contents out of conversation output. This is the common access layer for the other QDash user-facing skills.
...
```bash
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py config-path
```
...
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local task-results --limit 20 --status success
...
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local qubit-latest --task t1
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local issues --limit 20
...
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local ai-reviews --limit 20
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local provenance-stats
uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py --profile local timeseries --parameter t1 --start-at 2026-06-01T00:00:00Z --end-at 2026-06-08T00:00:00Z
```
...
```bash
uv run --with httpx --with pydantic python skills/qdash/qdash-core/scripts/qdash_query.py --profile local chips
```
...
2. Prefer `uv run --with qdash-client python skills/qdash/qdash-core/scripts/qdash_query.py ...` for supported read-only operations because it handles profile loading, auth headers, project headers, model serialization, and error formatting through the official client.
